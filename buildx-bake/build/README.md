...
steps:
  ...
  - uses: clean-botix/github-actions/buildx-bake/build@v5.1.0
    with:
...
`qemu` defaults to `auto`: compare `matrix.platform` with the Docker daemon's OS/architecture and skip setup for a native build. Foreign or nonbaseline variants retain QEMU setup before Buildx. Use `qemu: 'true'` to force the previous install-all behavior, including native output images with foreign execution stages (`FROM --platform=...`), or when `bake-set` overrides the matrix platform. Use `qemu: 'false'` for caller-managed emulation or a build that does not execute foreign binaries. Explicit modes bypass platform detection.
